...
const typeNum = (arr) => {
  let filteredArr = arr.filter(number => typeof(number) === "number");
  return filteredArr;
};
...
const containsAnd = (arr) => {
  let containsAnd = arr.filter(string => string.includes('and'));
  return containsAnd;
};
...
const oddValues = (arr) => {
  let oddValueArr = arr.filter(num => (num % 2));
  return oddValueArr;
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  let pureArray = arr.filter(num => !(forbiddenValues.includes(num)))
  return pureArray;
};
